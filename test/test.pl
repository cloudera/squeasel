#!/usr/bin/env perl
# This script is used to test Squeasel web server
...
my $config = 'squeasel.conf';
my $exe_ext = on_windows() ? '.exe' : '';
my $squeasel_exe = '.' . $dir_separator . 'squeasel' . $exe_ext;
...
# Make sure we export only symbols that start with "sq_", and keep local
# symbols static.
if ($^O =~ /darwin|bsd|linux/) {
  my $out = `(cc -c squeasel.c && nm squeasel.o) | grep ' T '`;
  foreach (split /\n/, $out) {
    /T\s+_?sq_.+/ or fail("Exported symbol $_")
  }
}
...
spawn("$squeasel_exe -listening_ports 127.0.0.1:$port");
...
unless (scalar(@ARGV) > 0 and $ARGV[0] eq "basic_tests") {
  ...
  unlink $path;
  system("$squeasel_exe -A $path a b c") == 0
    or fail("Cannot add user in a passwd file");
  system("$squeasel_exe -A $path a b c2") == 0
...
sub do_PUT_test {
  # This only works because squeasel currently doesn't look at the nonce.
