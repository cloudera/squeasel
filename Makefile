...
# squeasel directory, and then add $(LUA_SOURCES) to CFLAGS
LUA         = lua-5.2.1/src
...
ALL_SOURCES = main.c squeasel.c build/sqlite3.c build/lsqlite3.c \
...
# Using Visual Studio 6.0. To build Squeasel:
...
solaris:
	$(CC) squeasel.c main.c -lnsl -lsocket -o $(PROG) $(CFLAGS)
...
Squeasel: squeasel.c main.c
	$(CC) squeasel.c main.c build/lsqlite3.c build/sqlite3.c \
          -DUSE_COCOA $(CFLAGS) $(FLAGS) -mmacosx-version-min=10.4 \
          $(YASSL_SOURCES) $(LUA_SOURCES) \
          -framework Cocoa -ObjC -arch i386 -arch x86_64 -o Squeasel
...
mingw:
	...
	$(CC) $(MINGWOPT) squeasel.c -lws2_32 \
		-shared -Wl,--out-implib=$(PROG).lib -o $(PROG).dll
	$(CC) $(MINGWOPT) squeasel.c main.c build\res.o \
...
cygwin:
	windres ./build/res.rc ./build/res.o
	$(CC) $(CYGWINOPT) squeasel.c -lws2_32 \
		-shared -Wl,--out-implib=$(PROG).lib -o $(PROG).dll
	$(CC) $(CYGWINOPT) -Ibuild squeasel.c main.c ./build/res.o \
...
tarball: clean
	F=squeasel-`perl -lne '/define\s+MONGOOSE_VERSION\s+"(\S+)"/ and print $$1' squeasel.c`.tgz ; cd .. && tar -czf x squeasel/{LICENSE,Makefile,examples,test,build,*.[ch],*.md} && mv x squeasel/$$F

release: tarball cocoa
	wine make windows
	V=`perl -lne '/define\s+MONGOOSE_VERSION\s+"(\S+)"/ and print $$1' squeasel.c`; upx squeasel.exe; cp squeasel.exe squeasel-$$V.exe; cp squeasel.exe squeasel_php_bundle/; zip -r squeasel_php_bundle_$$V.zip squeasel_php_bundle/
